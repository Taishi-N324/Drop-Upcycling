...
#$ -l node_f=16
#$ -l h_rt=96:00:00
#$ -o outputs/upcycle/mixtral-8×152M_torch_rand_002_btx/$JOB_ID
#$ -e outputs/upcycle/mixtral-8×152M_torch_rand_002_btx/$JOB_ID
#$ -p -5
...
TOKENIZER_MODEL=/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x152M-btx_torch_rand_002/tokenizer.model
CHECKPOINT_DIR=/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x152M-btx_torch_rand_002
CHECKPOINT_SAVE_DIR="/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x152M-btx_torch_rand_002_zero3/lr_${LR}-minlr_${MIN_LR}_warmup_${LR_WARMUP_STEPS}_seq_${SEQ_LENGTH}"
...
JOB_NAME="upcycle-8×152M-torch_rand_002_btx-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
