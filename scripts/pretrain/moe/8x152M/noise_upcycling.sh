...
#$ -l node_f=16
#$ -l h_rt=48:00:00
#$ -o outputs/upcycle/Mixtral-8x152M-shuffle_torch_rand_002_iter_0477000_random_rand_init_0.50_noise/$JOB_ID
#$ -e outputs/upcycle/Mixtral-8x152M-shuffle_torch_rand_002_iter_0477000_random_rand_init_0.50_noise/$JOB_ID
#$ -p -5
...
TOKENIZER_MODEL=/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x152M-shuffle_torch_rand_002_iter_0477000_random_rand_init_0.50_noise/tokenizer.model
CHECKPOINT_DIR=/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x152M-shuffle_torch_rand_002_iter_0477000_random_rand_init_0.50_noise/
CHECKPOINT_SAVE_DIR="/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x152M-shuffle_torch_rand_002_iter_0477000_random_rand_init_0.50_noise_zero3/lr_${LR}-minlr_${MIN_LR}_warmup_${LR_WARMUP_STEPS}_seq_${SEQ_LENGTH}"
...
JOB_NAME="upcycle-Mixtral-8x152M-shuffle_torch_rand_002_iter_0477000_random_rand_init_0.50_noise-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
