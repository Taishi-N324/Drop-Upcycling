...
#$ -l node_f=16
#$ -l h_rt=129:00:00
#$ -o outputs/from_scratch_Mixtral-8x3.78B/$JOB_ID
#$ -e outputs/from_scratch_Mixtral-8x3.78B/$JOB_ID
#$ -p -5
...
TOKENIZER_MODEL=/gs/bs/tgh-NII-LLM/checkpoints/from_scratch_Mixtral-8x3.78B/tokenizer.model
CHECKPOINT_DIR=/gs/bs/tgh-NII-LLM/checkpoints/from_scratch_Mixtral-8x3.78B
CHECKPOINT_SAVE_DIR="/gs/bs/tgh-NII-LLM/checkpoints/from_scratch_Mixtral-8x3.78B_zero3/lr_${LR}-minlr_${MIN_LR}_warmup_${LR_WARMUP_STEPS}_seq_${SEQ_LENGTH}"
...
JOB_NAME="from_scratch_Mixtral-8x3.78B-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
